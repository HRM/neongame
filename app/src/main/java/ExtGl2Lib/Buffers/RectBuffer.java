...
public class RectBuffer {
    private int comSize=5000;
    private int indSize=5000;
    ...
    public void addRect(float[] scale, float[] pos, float rot){
        ...
        setCol( GL2F.getCol() );
        for(int i=0;i<4;i++){
            w1[i*4]*=scale[0]+GL2F.getFade()+GL2F.getThickness();
            w1[i*4+1]*=scale[1]+GL2F.getFade()+GL2F.getThickness();
            w1[i*4+2]=w1[i*4];
            w1[i*4+3]=w1[i*4+1];
            if(rot!=0) {
    ...
    public void drawOut(){
        ...
        compCol.position(0);
        ind.position(0);
        GL2F.drawRectMD( comp1,comp2,compCol,ind,currPos*6 );
        reset();
    }
